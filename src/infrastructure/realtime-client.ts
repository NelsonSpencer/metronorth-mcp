...
export class MetroNorthRealtime {
  constructor() {
    logger.info(
      "Metro-North realtime client initialized (public API - no key required)",
    );
  }
  ...
  private async fetchTripUpdates(): Promise<FeedMessage | null> {
    try {
      const response = await axios.get<ArrayBuffer>(GTFS_REALTIME_URL, {
        timeout: 15000,
      ...
      return decodeGtfsRealtimeFeed(new Uint8Array(response.data));
    } catch (error) {
  ...
  private async fetchAlerts(): Promise<FeedMessage | null> {
    try {
      const response = await axios.get<ArrayBuffer>(GTFS_ALERTS_URL, {
        timeout: 15000,
        responseType: "arraybuffer",
        headers: {
          "User-Agent": packageMetadata.userAgent,
  ...
  async getServiceAlerts(): Promise<ServiceAlert[]> {
      ...
      if (entity.alert) {
        ...
        if (mnrEntities.length > 0 || !a.informedEntity?.length) {
          alerts.push({
            ...
            url: getText(a.url),
            active_period_start: a.activePeriod?.[0]?.start
              ? parseInt(a.activePeriod[0].start)
              : null,
            active_period_end: a.activePeriod?.[0]?.end
              ? parseInt(a.activePeriod[0].end)
              : null,
    ...
    logger.info(
      { count: alerts.length },
      "Fetched service alerts from MTA API",
    );
  ...
  async getDelayForTripAtStop(
    tripId: string,
    stopId: string,
    trainNumber?: string,
  ): Promise<number | null> {
    const updates = await this.getTripUpdates();
    return this.getDelayForTripAtStopFromUpdates(
      updates,
      tripId,
      stopId,
      trainNumber,
  ...
  getDelayForTripAtStopFromUpdates(
    ...
    if (
      stopUpdate.departure_delay !== null &&
      stopUpdate.departure_delay !== undefined
    ) {
      return stopUpdate.departure_delay;
    }
    if (
      stopUpdate.arrival_delay !== null &&
      stopUpdate.arrival_delay !== undefined
    ) {
  ...
  private findTripUpdate(
    ...
    return (
      (trainNumber
        ...
        : undefined) || updates.find((u) => u.trip_id === tripId)
    );
  }
}
