...
async function main() {
  try {
    ...
    const routeSchedule = await callTool('get_route_schedule', {
      route_name: 'Hudson',
      direction: 'outbound',
    });
    ...
    if (!routeSchedule.trips?.length) {
      throw new Error(
        'No outbound Hudson schedule found. Run npm run gtfs:update and try again.'
      );
    }

    const allTripsPointToGrandCentral = routeSchedule.trips.every(
      (trip) => trip.destination === 'Grand Central'
    );
    if (allTripsPointToGrandCentral) {
      throw new Error('Outbound Hudson trips all point back to Grand Central.');
    }
    ...
    console.log(
      `Sample outbound destinations: ${routeSchedule.trips
        .slice(0, 3)
